...
const CreateProfile = ({ open, handleClose }) => {
  ...
  const editUser = async () => {
    ...
    await fetch(USER_EDIT_ENDPOINT, {
      method: "PATCH",
      headers: {
  ...
  return (
    <Dialog open={open}>
      ...
      <Box sx={{ mx: 3 }}>
        ...
        >
          <Box>
            <InputLabel sx={{ mb: 1, ml: 0.5 }}>Name:</InputLabel>
            <TextField variant="outlined" value={profile.name} error={error.name} helperText={error.name} onChange={(e) => setProfile((x) => ({ ...x, name: e.target.value }))} />
          </Box>
          <Box>
            <InputLabel sx={{ mb: 1, ml: 0.5 }}>Role:</InputLabel>
            <TextField
              disabled
              InputProps={{
                readOnly: true,
        ...
        <Box>
          <InputLabel sx={{ mb: 1, ml: 0.5 }}>Contact:</InputLabel>
          <TextField
            variant="outlined"
            fullWidth
            error={error.contact}
